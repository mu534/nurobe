...
const HOTEL_EMAIL = process.env.HOTEL_EMAIL || "mudassirnajimuddin@gmail.com";

// ================= Helper — throws if Resend returns an error =================
async function sendEmail(payload: Parameters<typeof resend.emails.send>[0]) {
  const { data, error } = await resend.emails.send(payload);
  if (error) {
    // Resend returns error object instead of throwing — we throw manually
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
        ...
        <div style="padding: 32px;">
          <p style="font-size: 16px;">Dear <strong>${data.guestName}</strong>,</p>
          <p style="color: #4b5563;">Your booking has been cancelled.</p>
